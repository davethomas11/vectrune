use crate::core::{AppState, extract_auth_configs, execute_steps, jwt_auth};
...
pub async fn build_rest_router(state: AppState, verbose: bool) -> Router {
    ...
    for section in &state.doc.sections {
        if section.path.first().map(|s| s.as_str()) == Some("Frontend") {
            if let Some(Value::String(frontend_type)) = section.kv.get("type") {
                if frontend_type == "web" {
                    let fe_path = section.kv.get("path").and_then(|v| v.as_str()).unwrap_or("/");
                    let layout = section.kv.get("layout").and_then(|v| v.as_str()).unwrap_or("");
                    if layout == "crud_powered" {
                        let wpath = if fe_path == "%ROOT%" { "/" } else { fe_path };
                        let state_clone = state.clone();
                        let name = section.kv.get("name").map(|v| v.to_string()).unwrap_or_default();
                        router = router.route(
                            wpath,
        ...
        if section.path.first().map(|s| s.as_str()) == Some("Route") {
            ...
            let method = section.path.get(1).map(|s| s.as_str()).unwrap_or("GET").to_uppercase();
            let path_template = section.path.iter().skip(2).map(|s| s.as_str()).collect::<Vec<_>>().join("/");
            ...
            if method == "CRUD" {
                for m in &["GET", "POST", "PUT", "DELETE"] {
                    for &with_id in &[false, true] {
                        let path = if with_id { format!("{}/{}", axum_path, "{id}") } else { axum_path.clone() };
