...
pub fn handle_merge(input_doc: &RuneDocument, spec: &str) -> Result<RuneDocument, String> {
    ...
    let (base_file, selector) = spec.split_once('@')
...
fn apply_merge(base_doc: &mut RuneDocument, input_doc: &RuneDocument, selector: &str) -> Result<(), String> {
...
fn update_at_path(
    ...
    if part.starts_with('(') && part.ends_with(')') {
        ...
        if inner.contains('|') {
            ...
            let (target_part, rest) = inner.split_once(" on ")
                .ok_or("Invalid merge instruction syntax: missing 'on'")?;
            let (val_field, source_key) = rest.split_once(" from ")
            ...
            if !source_values.is_empty() {
                for section in &mut base_doc.sections {
                    if section.path == current_path {
                        ...
                        if let Some(t_val) = target_val {
                            ...
                            section.series.insert(t_val.to_string(), source_values.clone());
                        } else {
                            section.series.insert(key_field.to_string(), source_values.clone());
                        }
                    }
                }
                ...
                for section in &mut base_doc.sections {
                    ...
                    let is_parent_match = section.path.len() > 0 && section.path[..section.path.len()-1] == current_path;
                    ...
                    if is_exact_match || is_parent_match || is_series_match {
                        ...
                        for (series_name, series) in &mut section.series {
                            if is_series_match {
                                let last_part = &current_path[current_path.len()-1];
                                if series_name != last_part { continue; }
                            }
                            ...
                            for val in series {
                                if let Value::Map(map) = val {
                                    if let Some(Value::String(n)) = map.get(key_field) {
                                        if let Some(t_val) = target_val {
                                             if n == t_val {
                                                 map.insert(val_field.to_string(), Value::List(source_values.clone()));
                                                 matches_found = true;
                                             }
                                        }
                                    }
                        ...
                        for record in &mut section.records {
                            if let Some(Value::String(n)) = record.kv.get(key_field) {
                                if let Some(t_val) = target_val {
                                    if n == t_val {
                                        record.kv.insert(val_field.to_string(), Value::List(source_values.clone()));
                                        matches_found = true;
                                    }
                                }
                            }
                    ...
                    if section.path == current_path {
                         if let Some(t_val) = target_val {
                             ...
                             if section.kv.contains_key(t_val) {
                                 section.kv.insert(t_val.to_string(), Value::List(source_values.clone()));
                                 matches_found = true;
                             }
            ...
            if !matches_found {
                ...
                if !source_values.is_empty() {
                    ...
                    for section in &mut base_doc.sections {
                        if section.path == current_path {
                             section.series.insert(key_to_update.to_string(), source_values.clone());
                        }
                    }
                }
            }
        ...
        for section in &base_doc.sections {
            if section.path.len() > current_path.len() && section.path[..current_path.len()] == current_path {
