...
";
    ...
    let output = Command::new("cargo")
        .args(&[
            "run", "--",
            "-i", "yaml", input.path_str(),
            "--merge-with", &format!("{}@environment.preview.[].(name=allowedIps on value from Ips)", base.path_str()),
            "-o", "yaml"
        ])
        .output()
...
";
    ...
    let output = Command::new("cargo")
        .args(&[
            "run", "--",
            input.path_str(),
            "--merge-with", &format!("{}@Roles", base.path_str()),
            "-o", "json"
        ])
        .output()
        .expect("failed to execute process");
...
";
    ...
    let output = Command::new("cargo")
        .args(&[
            "run", "--",
            "-i", "json", input.path_str(),
            "--merge-with", &format!("{}@api", base.path_str()),
            "-o", "yaml"
        ])
        .output()
        .expect("failed to execute process");
